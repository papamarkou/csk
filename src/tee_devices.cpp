...
boost::filesystem::ofstream errorOfstream;
boost::filesystem::ofstream logOfstream;

const TeeDevice errorTeeDevice(std::cerr, errorOfstream);
const TeeDevice logTeeDevice(std::cout, logOfstream);

TeeStream errorTee(errorTeeDevice);
TeeStream logTee(logTeeDevice);
