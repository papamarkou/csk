...
double negativeBernoulliMap(const double x, const std::vector<double> &pars) {
  double y;

  // pars(1) = lower bound, pars(2) = upper bound, pars(3) = non-centrality
  if ((pars[0]<=x) && (x<pars[1]))
    y = ((pars[0]-pars[2])*x+(pars[1]*pars[2]-pow(pars[0], 2)))/
      (pars[1]-pars[0]);
  else if ((pars[1]<=x) && (x<=pars[2]))
    y = ((pars[0]-pars[2])*x+(pow(pars[2], 2)-pars[0]*pars[1]))/
      (pars[2]-pars[1]);
  else
    throw NotElement(std::string("Error in \"negativeBernoulliMap\":\n ") +
      "The argument passed to the map does not belong to the map's domain");

  return y;
}

void generateNegativeBernoulliSpreading(
  boost::numeric::ublas::vector<double> &spr, const std::vector<double> &pars) {
  spr(0) =
    boost::random::uniform_real_distribution<>(pars[0], pars[2])(rng);
  for (unsigned int i=1; i<spr.size(); ++i) {
    spr(i) = negativeBernoulliMap(spr(i-1), pars);
  }
}
